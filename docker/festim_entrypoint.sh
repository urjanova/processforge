...
# the unprivileged MAMBA_USER and may write run artifacts there when
# PROCESSFORGE_OUTPUT_DIR points at /data (the API server defaults to /data
# when that variable is unset). Ensure /data is writable by that user, then
# drop privileges and exec the server/command.
set -euo pipefail

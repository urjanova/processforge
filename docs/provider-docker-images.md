...
**What happens:** `pf run` (and `pf apply`) reuses the `docker-compose.yml` already generated by `pf init`, starts the OpenMC container on port 9001, POSTs your simulation, saves results to `outputs/`, and stops the container. Pass `--keep-containers` to leave the container running after the run.
